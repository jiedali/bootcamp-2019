...
names(tmp)
tmp <- select(merged_energy, starts_with('bio'))
names(tmp)
...
```{r}
merged_energy %>% 
  summarize(mean_small_hydro = mean(merged_energy$small_hydro, na.rm = T),
            mean_large_hydro = mean(merged_energy$large_hydro, na.rm = T),
            mean_biogas = mean(merged_energy$biogas, na.rm = T),
            biomass = mean(merged_energy$biomass, na.rm = T)
            )
```
...
generation_df <- read_csv(here::here("data", "ca_energy_generation.csv"))
...
head(generation_dt)
## Create a new column "total hydro" which is 
generation_dt[, total_hydro:=small_hydro+large_hydro]
head(generation_dt)
generation_dt[, mean_nuclear:=mean(nuclear)]
generation_dt[, mean_biogas:=mean(biogas)]
head(generation_dt)
## Or, generate a new table with the mean values
generation_dt[,.(mean(nuclear), mean(biogas))]
## Group, row filtering and column manipulation
## find the median solar generation by hour
generation_dt[,.(median(solar)), by= .(hour(datetime))]
## 
generation_dt[,day := as_date(datetime)]
generation_dt[,log_output :=log(value)]
generation_dt[,per_output :=output/sum(value, na.rm=T), by = day]
head(generation_dt)
